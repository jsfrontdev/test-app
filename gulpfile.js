...
gulp.task('stylus-include', function() {
	return gulp.src('app/common/ather/*.styl')
		.pipe(foreach(function(stream, file) {
			console.log('styl-file:', file.basename);
			return merge(gulp.src([
				
				'app/common/ather/*.styl'
			...
			.pipe(gulpif(production, cssnano()))
			.pipe(gulpif(!production, sourcemaps.write()))
			.pipe(rename({ basename: file.basename.split('.')[0] }))
		}))
		.pipe(gulp.dest('dist/css/include'))
		.pipe(browserSync.reload({ stream: true }))
});
...
gulp.task('build', gulp.series(function(done) { production = true; done(); }, 'clean:dist', gulp.parallel('stylus-include', 'pug', 'stylus', 'js', 'images')));

gulp.task('default', gulp.series('clean:dist', gulp.parallel('stylus-include', 'pug', 'stylus', 'js', 'images', 'libs', 'fonts'), function server(done) {
	...
	gulp.watch('app/common/ather/*.styl', gulp.series('stylus-include'));
